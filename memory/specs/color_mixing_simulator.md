...
フラットな結果色と選択メッシュで結果を確認できるようにする。配合は名前付きの
...
マテリアルを持つ。染料行はEnabled、名前、校正色、校正濃度（滴/mL）、
...
する。結果は材質球をレンダリングせず、通常のカラースウォッチで表示する。入力変更
は、そのプロファイルの既存マテリアルへ即時反映する。プロファイル追加時に専用
マテリアルを作り、削除時は適用済みオブジェクトを壊さないようマテリアル自体は
削除しない。
